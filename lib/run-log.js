...
export function createRunLog({ source, trigger } = {}) {
  ...
  const api = {
    ...
    // truth in that order too).
    abandon(reason) {
      if (record.status !== 'running') return Promise.resolve();
      return api.finish('failed', { error: String(reason) });
    },
    ...
    armWatchdog(maxDurationMs, marginMs = 15_000) {
      clearTimeout(watchdogTimer);
      watchdogTimer = setTimeout(() => {
        const lastStep = record.events[record.events.length - 1]?.step;
        api.abandon(
          `Timed out — function hit its ${Math.round(maxDurationMs / 1000)}s limit` +
          (lastStep ? ` during "${lastStep}"` : '')
        );
      }, Math.max(maxDurationMs - marginMs, 1_000));
      watchdogTimer.unref?.();
    },
  };
  return api;
