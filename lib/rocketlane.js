...
//   2. The two files in a submission upload within a fraction of a second of
//      each other, in non-deterministic order — so we return the whole
//      submission cluster and let the caller pick the card art by image
//      dimensions (the card art is 1536-wide; the icon/logo is tiny). Neither
//      upload order nor byte size is reliable (an .ai icon can outweigh the
//      .ai card art); pixel area is.
...
// Resolve the files of the latest form submission on a project. Returns
// { taskId, submission: [{ attachmentId, filename, createdAt }] } (newest
// submission, files ordered oldest→newest) or null when no card-art task /
// attachments exist (caller treats null as "nothing to do"). The caller
// downloads the files and picks the card art by image dimensions.
export async function resolveLatestSubmission(projectId) {
