...
// Fetch a single task by ID. Lets the webhook derive projectId/projectName
// when the automation only forwards the task identifier.
export async function getTaskById(taskId) {
  const res = await fetch(`${V1_0}/tasks/${taskId}`, { headers: authHeaders() });
  if (!res.ok) {
    throw Object.assign(
      new Error(`Rocketlane task lookup failed (${taskId}): ${res.status}`),
      { step: 'rocketlane' }
    );
  }
  return res.json();
