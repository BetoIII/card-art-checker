...
export async function runAnalysis({ file, fileName, backFile, backFileName, cardType, onProgress }) {
  ...
  if (process.env.SPEC_SCRIPT_FILE_ID) {
    resources.push({
      ...
      mount_path: '/mnt/session/scripts/check_technical_specs.py',
    });
  }
  ...
  const pdfBuffer = await generatePdfReport(pdfImageBuffer, results, { cardType: resolvedCardType, hasBack });
