...
function buildTechSpecPrompt(hasBack, frontExt = '.ai', backExt = '.ai') {
  ...
  // The cp makes the rendered previews downloadable after the run: files
  // written under /mnt/session/outputs/ are indexed into the Files API
  // (scope_id = session), which is where generateResultsPdfViaSpecCheck
  // fetches them from. Silent no-op for raster fronts (no *_render.png).
...
mkdir -p /mnt/session/outputs && cp /mnt/session/uploads/*_render.png /mnt/session/outputs/ 2>/dev/null || true
\`\`\`
...
function buildPhysicalVisualPrompt(techJson, hasBack) {
  ...
  // Front zoom crops written by the spec script next to the rendered preview
  // (absent until the uploaded script includes crop emission — degrade to no
  // crop block rather than referencing files that don't exist).
  const cropDescriptions = {
    brand_mark: 'the Visa Brand Mark corner at 2× zoom — use for mark legibility, color, contrast, and the product identifier',
    issuer: 'the issuer-logo corner at 2× zoom',
    lower_left: 'the lower-left zone at native resolution',
  };
  const zoomCrops = techJson?.front?.zoom_crops || {};
  const cropBlock = Object.keys(zoomCrops).length
    ? `\nPre-rendered zoom crops of the front are available — \`read\` these instead of cropping or zooming yourself:\n${Object.entries(zoomCrops).map(([name, p]) => `- ${p} — ${cropDescriptions[name] || 'zoom crop'}`).join('\n')}\n`
    : '';
...
// Download one file from the session's Files-API scope by exact filename.
// The SDK's files methods build their own anthropic-beta header (files-api),
// which replaces the client-level managed-agents default — pass it explicitly
// or the scope_id filter is rejected with "unknown field". Files written to
// /mnt/session/outputs/ appear in the list after a 1-3s indexing lag past
// session idle, so retry before giving up. Returns null when not found.
const MANAGED_AGENTS_BETA = 'managed-agents-2026-04-01';

async function downloadSessionFile(sessionId, filename, { attempts = 3 } = {}) {
  if (!filename) return null;
  let match = null;
  for (let attempt = 0; attempt < attempts && !match; attempt++) {
    if (attempt > 0) await new Promise(r => setTimeout(r, 1500));
    for await (const f of getAnthropic().beta.files.list({ scope_id: sessionId, betas: [MANAGED_AGENTS_BETA] })) {
      if (f.filename !== filename || f.downloadable === false) continue;
      if (!match || (f.created_at || '') > (match.created_at || '')) match = f;
    }
  }
  if (!match) return null;
  const response = await getAnthropic().beta.files.download(match.id, { betas: [MANAGED_AGENTS_BETA] });
...
// session spends its entire budget on visual inspection. Physical tech specs
// stay in-session (.ai/.eps rendering needs the sandbox's Ghostscript), but
// the annotated physical report renders off-session too: it only composes
// the preview PNGs Turn 1 already rendered.
...
export async function runAnalysis({ file, fileName, backFile, backFileName, cardType, onProgress, deadlineAt }) {
  ...
  if (resolvedCardType === 'virtual') {
    ...
    const [uploadedFront, uploadedBack] = await Promise.all([
      getAnthropic().beta.files.upload({
        file: new File([file], `front${physicalFrontExt}`, { type: frontMime }),
      }),
      hasBack
        ? getAnthropic().beta.files.upload({
            file: new File([backFile], `back${physicalBackExt}`, { type: mimeForPhysicalExt(physicalBackExt) }),
          })
    ...
    // Physical-only: the in-session tech-specs turn runs the uploaded spec
    // script inside the sandbox (Ghostscript). Virtual runs — and both card
    // types' annotated reports — use the bundled copy in api/spec-check.py
    // instead, with no Files-API script dependency.
  ...
  await getAnthropic().beta.sessions.events.send(session.id, {
    events: [{
      type: 'user.message',
      content: [{ type: 'text', text: buildVisualPrompt(techJson, resolvedCardType, hasBack, cropPaths) }],
    }],
  });
  ...
  try {
    ...
    if (resolvedCardType === 'virtual') {
      ...
      const frontName = (techJson?.front?.rendered_preview_path || '').split('/').pop();
      const frontPreview = physicalFrontExt === '.png' ? file : await downloadSessionFile(session.id, frontName);
      if (!frontPreview) {
        throw new Error(`front preview ${frontName || '(unknown)'} not found in session files`);
      }
      let backPreview = null;
      if (hasBack) {
        const backName = (techJson?.back?.rendered_preview_path || '').split('/').pop();
        backPreview = physicalBackExt === '.png' ? backFile : await downloadSessionFile(session.id, backName);
        // A missing back preview renders as a note strip, not a failure.
      }
      renderRes = await callSpecCheck('render-physical', {
        tech_results: techJson,
        previews: {
          front: frontPreview.toString('base64'),
          ...(backPreview ? { back: backPreview.toString('base64') } : {}),
        },
        visual_results: visualResults,
      });
    }
    ...
    if (sourceBlob) blobDel(sourceBlob.url).catch(err => console.warn('transport blob cleanup failed:', err?.message || err));
  }
  if (!pdfBuffer) {
