...
// Virtual: card art with the 56px boundary, sample PAN overlay, RGB swatches,
// numbered markers at marker_x/marker_y, and the spec + visual check tables.
// Physical: front/back review panes with the scaled 56px boundary, per-side
// markers (marker_side), File Info panels, and per-side spec tables.
// The Node side owns the JSON handoff so the agent only executes commands.
function buildResultsPdfPrompt(visualResultsJson, { cardType = 'virtual', frontExt = '.ai', backExt = '.ai', hasBack = false } = {}) {
  let target = '/mnt/session/uploads/card-art.png --card-type virtual';
  let gsGuard = '';
  if (cardType === 'physical') {
    const backArg = hasBack ? ` --back /mnt/session/uploads/back${backExt}` : '';
    target = `/mnt/session/uploads/front${frontExt} --card-type physical${backArg}`;
    ...
    if (frontExt !== '.png' || (hasBack && backExt !== '.png')) {
      gsGuard = 'which gs || (apt-get update -qq && apt-get install -y ghostscript)\n';
    }
  }
...
export async function runAnalysis({ file, fileName, backFile, backFileName, cardType, onProgress, deadlineAt }) {
  ...
  // Turn 3: have the agent render the annotated results PDF — the card art
  // (front/back panes for physical) with the 56px boundary and numbered
  ...
  if (remainingMs() < RESULTS_PDF_MIN_MS) {
