...
//
// The confidence rules in SYSTEM_PROMPT are mirrored in the standalone
// `slack-channel-identifier` skill (~/.claude/skills/), which runs the same
// judgment interactively. Change one, change the other.
//
...
Two signals:
1. Creation-time proximity — the channel's created time falls shortly AFTER the project's createdAt (usually within seconds). Start with a window of [project createdAt − 1 hour, createdAt + 48 hours]; widen stepwise (up to ~14 days after) only if the window is empty. Time proximity proves which channel was provisioned FOR this project. The window reaches back an hour only to absorb clock skew: the bot provisions AFTER the project, so a channel created BEFORE it has a negative delta, is weak evidence, and is rarely the answer.
...
Required cross-check before concluding: after the time-window query, ALWAYS also call list_rocketlane_channels with name_contains set to a distinctive squashed fragment of the project name (no time window). Then adjudicate every match it surfaces from OUTSIDE the time window by asking: could that channel plausibly be where THIS customer's team lives NOW?
- Yes — typically a LIVE channel whose current name fits this customer as well as or better than the window candidate does. The two signals conflict; channels are sometimes renamed across customers. You must NOT report high: report low with both channels as candidates and describe the conflict.
- No — typically an ARCHIVED channel belonging to a different company that merely shares a substring (e.g. a search for "blinka" also surfacing ext-blinkai-rain, a different customer, archived months earlier). That is a substring artifact, not a conflict: dismiss it, keep your confidence, and say in your reasoning why it was dismissed.
